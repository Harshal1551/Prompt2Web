...
import { BotIcon, EyeIcon, Loader2Icon, MessageSquareOff, SendIcon, UserIcon } from 'lucide-react';
                   ...
                   if(isMessage){
                    ...
                    return (
                        <div key={msg.id} className={`flex items-center gap-3 ${isUser ? "justify-end": "justify-start"}`}>
                            ...
                            <div className={`max-w-[80%] p-2 px-4 rounded-2xl shadow-sm text-sm text-sm mt-5 leading-relaxed ${isUser ? "bg-linear-to-r from-indigo-500 to-indigo-600 text-white rounded-tr-none" : "rounded-tl-none bg-gray-800 text-gray-100"}`}>
                                {msg.content}
                            </div>
                            {isUser && (
                                <div>
